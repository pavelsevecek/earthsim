...
void generateMountainRanges(uint32_t seed) {
    ...
    mountainRanges.reserve(size_t(count));
    for (int i = 0; i < count; ++i)
        mountainRanges.push_back({ value(-380, 380),
            value(-380, 380),
            value(0, pi),
            value(430, 780),
            value(105, 205),
...
Mat4 perspective(float aspect, float distance) {
    ...
    float nearPlane = std::max(0.001f, distance * 0.0001f),
          farPlane = std::max(6000.0f, distance + 4000.0f);
    return { f / aspect,
        0,
        0,
        0,
...
uint32_t hash(int x, int z) {
    uint32_t h = uint32_t(x) * 0x8da6b343u ^ uint32_t(z) * 0xd8163841u ^ terrainSeed * 0x9e3779b9u ^
                 0xcb1ab31fu;
    h ^= h >> 16;
    h *= 0x7feb352du;
    h ^= h >> 15;
...
GLuint compileShader(const std::filesystem::path& file, GLenum type) {
    startupDiagnostic("Compiling " + file.string());
...
GLuint program(const std::filesystem::path& directory, const std::string& name) {
    ...
    try {
        fs = compileShader(directory / (name + ".frag"), GL_FRAGMENT_SHADER);
    } catch (...) {
        glDeleteShader(vs);
        throw;
    }
...
struct Terrain {
    ...
    Terrain() {
        ...
        for (int z = 0; z <= cells; ++z)
            for (int x = 0; x <= cells; ++x) {
                ...
                Vec3 n = normalize({ height(px - step, pz) - height(px + step, pz),
                    2 * step,
                    height(px, pz - step) - height(px, pz + step) });
        ...
        auto bounds = std::minmax_element(heights.begin(), heights.end());
        minHeight = *bounds.first;
    ...
    void updateGeometry() {
        for (int z = 0; z <= cells; ++z)
            for (int x = 0; x <= cells; ++x) {
                int left = std::max(0, x - 1), right = std::min(cells, x + 1),
                    back = std::max(0, z - 1), front = std::min(cells, z + 1);
                size_t i = size_t(z * (cells + 1) + x);
                float dx = (heights[size_t(z * (cells + 1) + right)] -
                               heights[size_t(z * (cells + 1) + left)]) /
                           ((right - left) * step);
                float dz = (heights[size_t(front * (cells + 1) + x)] -
                               heights[size_t(back * (cells + 1) + x)]) /
                           ((front - back) * step);
                vertices[i].position.y = heights[i];
                vertices[i].normal = normalize({ -dx, 1, -dz });
            }
        auto bounds = std::minmax_element(heights.begin(), heights.end());
        minHeight = *bounds.first;
        maxHeight = *bounds.second;
        glBindBuffer(GL_ARRAY_BUFFER, vbo);
        glBufferSubData(
            GL_ARRAY_BUFFER, 0, GLsizeiptr(vertices.size() * sizeof(Vertex)), vertices.data());
    }
    ...
    void carveCrater(Vec3 center, float radius) {
        for (size_t i = 0; i < vertices.size(); ++i) {
            ...
            float bowl =
                r < radius
                    ? sphereCenterY - std::sqrt(std::max(0.0f, sphereRadius * sphereRadius - r * r))
    ...
    bool segmentHit(Vec3 start, Vec3 end, Vec3& hit) const {
        ...
        if (maxX < -1000 || minX > 1000 || maxZ < -1000 || minZ > 1000)
            return false;
        auto grid = [](float v) {
            return std::clamp(int(std::floor((v + 1000) / step)), 0, cells - 1);
        };
    ...
    float surface(float x, float z, Vec3& normal) const {
        ...
        float ha = heights[a], hb = heights[a + 1], hc = heights[a + cells + 1],
              hd = heights[a + cells + 2];
        float dx, dz, y;
        if (u + v <= 1) {
            dx = (hb - ha) / step;
            dz = (hc - ha) / step;
...
struct Volcanoes {
    ...
    struct GpuSimulation {
        ...
        explicit GpuSimulation(const std::filesystem::path& directory, const Terrain& terrain) {
            ...
            const GLsizeiptr sizes[] = { GLsizeiptr(capacity * sizeof(GpuParticle)),
                GLsizeiptr(capacity * 4 * sizeof(float)),
                GLsizeiptr(capacity * 4 * sizeof(float)),
                GLsizeiptr(capacity * sizeof(float)),
                GLsizeiptr(buckets * sizeof(int)),
                GLsizeiptr(capacity * sizeof(int)) };
        ...
        void spawn(const std::vector<GpuParticle>& records) {
            ...
            while (source < records.size()) {
                ...
                glBufferSubData(GL_SHADER_STORAGE_BUFFER,
                    ...
                    records.data() + source);
                cursor = (cursor + uint32_t(count)) % capacity;
                source += count;
            }
        ...
        void step(float dt, bool interactions, float lifetime, float waterLevel, float windSpeed) {
            ...
            run(0, std::max(capacity, buckets));
            run(1, capacity);
    ...
    Vec3 glow(float temperature) const {
        ...
        return blackbodyColors[low] * (1 - (index - low)) +
               blackbodyColors[low + 1] * (index - low);
    }
    ...
    explicit Volcanoes(const std::filesystem::path& directory, const Terrain& terrain)
        : gpu(directory, terrain) {
        Vec3 reference = blackbody(1600);
        float scale = 1 / std::max({ reference.x, reference.y, reference.z });
        for (size_t i = 0; i < blackbodyColors.size(); ++i)
            blackbodyColors[i] = blackbody(300 + 1500 * float(i) / 255) * scale;
        sprites.reserve(32);
        glGenTextures(1, &blackbodyTexture);
        glBindTexture(GL_TEXTURE_1D, blackbodyTexture);
        glTexImage1D(GL_TEXTURE_1D,
            0,
            GL_RGB32F,
            GLsizei(blackbodyColors.size()),
            0,
            GL_RGB,
            GL_FLOAT,
            blackbodyColors.data());
        ...
        glVertexAttribPointer(1,
            ...
            GL_FALSE,
            sizeof(Sprite),
            reinterpret_cast<void*>(offsetof(Sprite, size)));
        ...
        glVertexAttribPointer(2,
            ...
            GL_FALSE,
            sizeof(Sprite),
            reinterpret_cast<void*>(offsetof(Sprite, emissionColor)));
        glVertexAttribDivisor(0, 1);
        glVertexAttribDivisor(1, 1);
        glVertexAttribDivisor(2, 1);
    }
    ~Volcanoes() {
        glDeleteTextures(1, &blackbodyTexture);
        glDeleteBuffers(1, &vbo);
        glDeleteVertexArrays(1, &vao);
    ...
    void terrainChanged(Terrain& terrain) {
        ...
        for (auto& vent : vents) {
            Vec3 n;
            vent.y = terrain.surface(vent.x, vent.z, n) + sourceClearance;
        }
        for (auto& spring : springs) {
            Vec3 n;
            spring.y = terrain.surface(spring.x, spring.z, n) + sourceClearance;
        }
    }
    void impact(Terrain& terrain, Vec3 position) {
        ...
        for (size_t i = 0; i < ejectaCount; ++i) {
            ...
            Vec3 velocity{ std::cos(angle) * horizontalSpeed,
                speed * std::cos(pi / 4),
    ...
    void update(Terrain& terrain, double elapsed, float waterLevel, float windSpeed) {
        ...
        while (accumulator >= dt) {
            ...
            for (size_t i = 0; i < meteors.size();) {
                ...
                } else if (next.y < terrain.minHeight - 1000)
                    meteors.erase(meteors.begin() + i);
                else
                    ++i;
            }
            ...
            while (emission >= 1) {
                ...
                for (Vec3 vent : vents) {
                    ...
                    spawn.y =
                        std::max(spawn.y, ground + radius / std::max(normal.y, 0.25f) + 0.25f);
    ...
    void prepareDraw(const Mat4& vp,
        const Mat4& lightVp,
        GLuint shadowMap,
        GLuint reflectionTexture,
        const Mat4& reflectionVp,
        Vec3 right,
        Vec3 up,
        Vec3 eye,
        Vec3 sun,
        ...
        float time,
        bool reflectionCapture,
        bool clipEnabled,
        float clipHeight,
        float clipDirection) {
        glUseProgram(shader);
        glUniformMatrix4fv(glGetUniformLocation(shader, "viewProjection"), 1, GL_FALSE, vp.data());
        uniform(shader, "cameraRight", right);
        uniform(shader, "cameraUp", up);
        uniform(shader, "eye", eye);
        uniform(shader, "sunDirection", sun);
        ...
        glUniformMatrix4fv(
            glGetUniformLocation(shader, "lightViewProjection"), 1, GL_FALSE, lightVp.data());
        glActiveTexture(GL_TEXTURE5);
        glBindTexture(GL_TEXTURE_2D_ARRAY, shadowMap);
        glUniform1i(glGetUniformLocation(shader, "terrainShadowMap"), 5);
        glActiveTexture(GL_TEXTURE6);
        glBindTexture(GL_TEXTURE_2D, reflectionTexture);
        glUniform1i(glGetUniformLocation(shader, "reflectionTexture"), 6);
        glUniformMatrix4fv(glGetUniformLocation(shader, "reflectionViewProjection"),
            1,
            GL_FALSE,
            reflectionVp.data());
        uniform(shader, "time", time);
        glUniform1i(glGetUniformLocation(shader, "reflectionCapture"), reflectionCapture ? 1 : 0);
        glUniform1i(glGetUniformLocation(shader, "clipEnabled"), clipEnabled ? 1 : 0);
        uniform(shader, "clipHeight", clipHeight);
        uniform(shader, "clipDirection", clipDirection);
    ...
    void draw(const Mat4& vp,
        ...
        sprites.clear();
        for (Vec3 vent : vents)
            sprites.push_back({ vent + Vec3{ 0, 3.0f, 0 }, 5, 1, glow(1600) });
        for (const auto& meteor : meteors)
            sprites.push_back({ meteor.position, 8, 1, glow(1800) * 3 });
        if (!sprites.empty()) {
            glBufferData(GL_ARRAY_BUFFER,
                GLsizeiptr(sprites.size() * sizeof(Sprite)),
                sprites.data(),
                GL_STREAM_DRAW);
            glEnableVertexAttribArray(0);
            glEnableVertexAttribArray(1);
...
struct Lightning {
    ...
    explicit Lightning(const std::filesystem::path& directory) {
        ...
        glVertexAttribPointer(1,
            ...
            GL_FALSE,
            sizeof(Segment),
            reinterpret_cast<void*>(offsetof(Segment, end)));
        glVertexAttribDivisor(0, 1);
        glVertexAttribDivisor(1, 1);
    }
    ~Lightning() {
    ...
    void spawn(const Terrain& terrain, float cloudBase, float cloudTop) {
        Vec3 origin{ range(-820, 820),
            range(cloudBase + 0.33f * (cloudTop - cloudBase), cloudTop - 30.0f),
            range(-820, 820) };
        float targetX = std::clamp(origin.x + range(-180, 180), -980.0f, 980.0f);
        float targetZ = std::clamp(origin.z + range(-180, 180), -980.0f, 980.0f);
        Vec3 normal;
        Vec3 target{ targetX, terrain.surface(targetX, targetZ, normal) + 1.0f, targetZ };
        Strike strike;
    ...
    void update(const Terrain& terrain, double elapsed, float cloudBase, float cloudTop) {
        ...
        strikes.erase(std::remove_if(strikes.begin(),
                          strikes.end(),
        ...
        untilNext -= elapsed;
        while (untilNext <= 0) {
            spawn(terrain, cloudBase, cloudTop);
            untilNext += interval();
        }
    }
    void draw(const Mat4& vp,
        ...
        for (const auto& strike : strikes) {
            ...
            for (auto segment : strike.segments) {
                ...
                visibleSegments.push_back(segment);
            }
        }
        ...
        glUniformMatrix4fv(glGetUniformLocation(shader, "viewProjection"), 1, GL_FALSE, vp.data());
        uniform(shader, "eye", eye);
        uniform(shader, "cameraRight", cameraRight);
        glUniform1i(glGetUniformLocation(shader, "clipEnabled"), clipEnabled ? 1 : 0);
        uniform(shader, "clipHeight", clipHeight);
        uniform(shader, "clipDirection", clipDirection);
        glBindVertexArray(vao);
        glBindBuffer(GL_ARRAY_BUFFER, vbo);
        glBufferData(GL_ARRAY_BUFFER,
            GLsizeiptr(visibleSegments.size() * sizeof(Segment)),
...
struct Rain {
    ...
    struct alignas(16) GpuDrop {
        float positionAge[4], velocityState[4];
    };
    ...
    explicit Rain(const std::filesystem::path& directory) {
        ...
        try {
            shader = program(directory, "rain");
        } catch (...) {
            glDeleteProgram(compute);
            throw;
        }
        glGenBuffers(1, &drops);
        glBindBuffer(GL_SHADER_STORAGE_BUFFER, drops);
        std::vector<GpuDrop> inactive(capacity);
        glBufferData(GL_SHADER_STORAGE_BUFFER,
            GLsizeiptr(inactive.size() * sizeof(GpuDrop)),
            inactive.data(),
            GL_DYNAMIC_DRAW);
        glGenBuffers(1, &wetness);
        glBindBuffer(GL_SHADER_STORAGE_BUFFER, wetness);
    ...
    ~Rain() {
        glDeleteBuffers(1, &wetness);
        glDeleteBuffers(1, &drops);
        glDeleteVertexArrays(1, &vao);
        glDeleteProgram(shader);
        glDeleteProgram(compute);
    }
    void update(double elapsed,
        ...
        glBindTexture(GL_TEXTURE_3D, cloudDensity);
        glUniform1i(glGetUniformLocation(compute, "cloudDensityTexture"), 1);
        glUniform1ui(glGetUniformLocation(compute, "capacity"), capacity);
        uniform(compute, "intensity", cloudsEnabled ? intensity : 0.0f);
        uniform(compute, "cloudCoverage", cloudCoverage);
        uniform(compute, "waterLevel", waterLevel);
        uniform(compute, "windSpeed", windSpeed);
        uniform(compute, "cloudBase", cloudBase);
        uniform(compute, "cloudTop", cloudTop);
        uniform(compute, "time", time);
        uniform(compute, "eye", eye);
        glUniform1f(glGetUniformLocation(compute, "dt"), dt);
        auto run = [&](int pass, uint32_t count) {
            glUniform1i(glGetUniformLocation(compute, "pass"), pass);
            glDispatchCompute((count + 127) / 128, 1, 1);
        ...
        while (accumulator >= dt) {
            ...
            run(0, wetnessSize * wetnessSize);
            run(1, capacity);
        }
        glActiveTexture(GL_TEXTURE0);
    }
    void bindWetness() const {
        glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 7, wetness);
    }
    void draw(const Mat4& vp,
        Vec3 eye,
        Vec3 right,
...
struct UiRenderer {
    ...
    void draw(ImDrawData* data, int w, int h) {
        auto setup = [&]() {
            ...
            glUniform2f(glGetUniformLocation(shader, "displayPosition"),
                data->DisplayPos.x,
                data->DisplayPos.y);
            glUniform2f(glGetUniformLocation(shader, "displaySize"),
                data->DisplaySize.x,
                data->DisplaySize.y);
            glBindVertexArray(vao);
            glBindBuffer(GL_ARRAY_BUFFER, vbo);
        ...
        for (int i = 0; i < data->CmdListsCount; ++i) {
            ...
            glBufferData(GL_ARRAY_BUFFER,
                GLsizeiptr(list->VtxBuffer.Size * sizeof(ImDrawVert)),
                list->VtxBuffer.Data,
                GL_STREAM_DRAW);
            glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                GLsizeiptr(list->IdxBuffer.Size * sizeof(ImDrawIdx)),
                list->IdxBuffer.Data,
                GL_STREAM_DRAW);
            for (const auto& cmd : list->CmdBuffer) {
                ...
                float x1 =
                    std::clamp((cmd.ClipRect.x - data->DisplayPos.x) * data->FramebufferScale.x,
                        0.0f,
                        float(w));
                float y1 =
                    std::clamp((cmd.ClipRect.y - data->DisplayPos.y) * data->FramebufferScale.y,
                        0.0f,
                        float(h));
                float x2 =
                    std::clamp((cmd.ClipRect.z - data->DisplayPos.x) * data->FramebufferScale.x,
                        0.0f,
                        float(w));
                float y2 =
                    std::clamp((cmd.ClipRect.w - data->DisplayPos.y) * data->FramebufferScale.y,
                        0.0f,
                        float(h));
                if (x2 <= x1 || y2 <= y1)
                    continue;
                ...
                glDrawElementsBaseVertex(GL_TRIANGLES,
                    ...
                    reinterpret_cast<void*>(size_t(cmd.IdxOffset) * sizeof(ImDrawIdx)),
                    GLint(cmd.VtxOffset));
            }
        }
...
struct TerrainShadows {
    ...
    void render(const Terrain& terrain, Vec3 sun) {
        ...
        float lightDistance = extent + 700, nearPlane = 100,
              farPlane = lightDistance + extent + 100;
        ...
        for (int layer = 0; layer < 2; ++layer) {
            ...
            lightMatrices[layer] = multiply(projection,
                lookAt(Vec3{ 0, centerY, 0 } + direction * lightDistance, forward, right, up));
            glFramebufferTextureLayer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, depth, 0, layer);
            glClear(GL_DEPTH_BUFFER_BIT);
            glUniformMatrix4fv(glGetUniformLocation(shader, "lightViewProjection"),
                1,
                GL_FALSE,
                lightMatrices[layer].data());
    ...
    void bind(GLuint terrainProgram) const {
        glUniformMatrix4fv(glGetUniformLocation(terrainProgram, "lightViewProjection[0]"),
            2,
            GL_FALSE,
            lightMatrices[0].data());
...
struct Clouds {
    ...
    GLuint sceneFbo = 0, cloudFbo = 0, sceneColor = 0, sceneDepth = 0, sceneEmission = 0,
           cloudColor = 0, noiseTexture = 0;
    ...
    explicit Clouds(const std::filesystem::path& directory, GLuint terrainTexture)
        : terrainHeightTexture(terrainTexture) {
        ...
        glGenTextures(1, &sceneEmission);
        glGenTextures(1, &cloudColor);
        ...
        glGenTextures(2, densityVolumes.data());
        glGenTextures(2, velocityVolumes.data());
        glGenTextures(2, pressureVolumes.data());
        glGenTextures(1, &divergenceVolume);
        glGenBuffers(1, &vaporMoisture);
        glBindBuffer(GL_SHADER_STORAGE_BUFFER, vaporMoisture);
        glBufferData(GL_SHADER_STORAGE_BUFFER,
            GLsizeiptr(simulationX * simulationY * simulationZ * sizeof(uint32_t)),
            nullptr,
            GL_DYNAMIC_DRAW);
        ...
        for (GLuint texture : densityVolumes)
            allocateVolume(texture, GL_R16F, GL_RED);
        for (GLuint texture : velocityVolumes)
            allocateVolume(texture, GL_RGBA16F, GL_RGBA);
        for (GLuint texture : pressureVolumes)
            allocateVolume(texture, GL_R16F, GL_RED);
        allocateVolume(divergenceVolume, GL_R16F, GL_RED);
        glUseProgram(simulation);
        glUniform3i(
            glGetUniformLocation(simulation, "volumeSize"), simulationX, simulationY, simulationZ);
        uniform(simulation, "windSpeed", 10.0f);
        uniform(simulation, "cloudBase", 290.0f);
        uniform(simulation, "cloudTop", 530.0f);
        glActiveTexture(GL_TEXTURE4);
    ...
    ~Clouds() {
        ...
        GLuint textures[] = { sceneColor, sceneDepth, sceneEmission, cloudColor, noiseTexture };
        glDeleteTextures(5, textures);
        glDeleteTextures(2, densityVolumes.data());
        glDeleteTextures(2, velocityVolumes.data());
        glDeleteTextures(2, pressureVolumes.data());
        glDeleteTextures(1, &divergenceVolume);
        glDeleteBuffers(1, &vaporMoisture);
        glDeleteProgram(shader);
        glDeleteProgram(composite);
        glDeleteProgram(simulation);
        glDeleteProgram(vaporDeposition);
        glDeleteVertexArrays(1, &vao);
    }
    GLuint densityTexture() const {
        return densityVolumes[size_t(densityIndex)];
    }
    void clearDensity() {
        glUseProgram(simulation);
        glUniform3i(
    ...
    void update(double elapsed,
        ...
        while (simulationAccumulator >= step) {
            ...
            glUniform1ui(glGetUniformLocation(vaporDeposition, "particleCapacity"),
                Volcanoes::GpuSimulation::capacity);
            glUniform1ui(glGetUniformLocation(vaporDeposition, "voxelCount"), voxelCount);
            glUniform3i(glGetUniformLocation(vaporDeposition, "volumeSize"),
                simulationX,
                simulationY,
                simulationZ);
            uniform(vaporDeposition, "cloudBase", cloudBase);
            uniform(vaporDeposition, "cloudTop", cloudTop);
            ...
            glBindTexture(GL_TEXTURE_2D, terrainHeightTexture);
            glUniform1i(glGetUniformLocation(simulation, "terrainHeight"), 4);
            glUniform3i(glGetUniformLocation(simulation, "volumeSize"),
                simulationX,
                simulationY,
                simulationZ);
            uniform(simulation, "dt", step);
            uniform(simulation, "time", time);
            uniform(simulation, "windSpeed", windSpeed);
            uniform(simulation, "cloudBase", cloudBase);
            uniform(simulation, "cloudTop", cloudTop);
            std::array<float, 32> positions{}, velocities{};
            int meteorCount = 0;
            for (const auto& meteor : meteors) {
                if (meteorCount >= 8 || meteor.position.y < cloudBase - 120.0f ||
                    meteor.position.y > cloudTop + 120.0f ||
                    std::abs(meteor.position.x) > 2320.0f || std::abs(meteor.position.z) > 2320.0f)
                    continue;
            ...
            if (meteorCount > 0) {
                glUniform4fv(glGetUniformLocation(simulation, "meteorPositionRadius[0]"),
                    meteorCount,
                    positions.data());
                glUniform4fv(glGetUniformLocation(simulation, "meteorVelocityStrength[0]"),
                    meteorCount,
                    velocities.data());
            }
            ...
            auto run = [&](int pass, GLuint densityOut, GLuint velocityOut, GLuint scalarOut) {
                ...
                glUniform1i(glGetUniformLocation(simulation, "pass"), pass);
                glDispatchCompute(
                    (simulationX + 3) / 4, (simulationY + 3) / 4, (simulationZ + 3) / 4);
                glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT | GL_TEXTURE_FETCH_BARRIER_BIT);
            };
            ...
            run(1,
                densityVolumes[size_t(1 - densityIndex)],
                velocityVolumes[size_t(nextVelocity)],
                divergenceVolume);
            velocityIndex = nextVelocity;
            source(1, "velocityTexture", velocityVolumes[size_t(velocityIndex)]);
            run(2,
                densityVolumes[size_t(1 - densityIndex)],
                velocityVolumes[size_t(1 - velocityIndex)],
                divergenceVolume);
            ...
            for (int iteration = 0; iteration < 10; ++iteration) {
                ...
                run(3,
                    densityVolumes[size_t(1 - densityIndex)],
                    velocityVolumes[size_t(1 - velocityIndex)],
                    pressureVolumes[size_t(nextPressure)]);
            ...
            run(4,
                densityVolumes[size_t(1 - densityIndex)],
                velocityVolumes[size_t(projectedVelocity)],
                divergenceVolume);
            ...
            run(5,
                densityVolumes[size_t(nextDensity)],
                velocityVolumes[size_t(1 - velocityIndex)],
                divergenceVolume);
        ...
        if (w != width || h != height) {
            ...
            auto allocate = [](GLuint texture, int w, int h, bool depth) {
                ...
                glTexImage2D(GL_TEXTURE_2D,
                    ...
                    nullptr);
                glTexParameteri(
                    GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, depth ? GL_NEAREST : GL_LINEAR);
                glTexParameteri(
                    GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, depth ? GL_NEAREST : GL_LINEAR);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
            };
            allocate(sceneColor, w, h, false);
            allocate(sceneDepth, w, h, true);
            allocate(sceneEmission, w, h, false);
            ...
            glFramebufferTexture2D(
                GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, sceneColor, 0);
            glFramebufferTexture2D(
                GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, sceneEmission, 0);
            glFramebufferTexture2D(
                GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, sceneDepth, 0);
            if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
                throw std::runtime_error("Cloud scene framebuffer is incomplete.");
            glBindFramebuffer(GL_FRAMEBUFFER, cloudFbo);
            glFramebufferTexture2D(
                GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, cloudColor, 0);
            if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
    ...
    void draw(Vec3 eye,
        ...
        uniform(shader, "eye", eye);
        uniform(shader, "cameraForward", forward);
        uniform(shader, "cameraRight", right);
        uniform(shader, "cameraUp", up);
        uniform(shader, "sunDirection", sun);
        uniform(shader, "fogColor", fog);
        uniform(shader, "daylight", daylight);
        uniform(shader, "atmosphereOpacity", atmosphereOpacity);
        uniform(shader, "cloudOpacity", cloudOpacity);
        uniform(shader, "time", time);
        uniform(shader, "cloudCoverage", coverage);
        uniform(shader, "windSpeed", windSpeed);
        uniform(shader, "cloudBase", cloudBase);
        uniform(shader, "cloudTop", cloudTop);
        uniform(shader, "aspect", float(width) / height);
        uniform(shader, "tanHalfFov", std::tan(pi / 8));
        Mat4 projection = perspective(float(width) / height, distance);
        glUniform2f(
            glGetUniformLocation(shader, "depthProjection"), projection[10], projection[14]);
        ...
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, cloudColor);
        glUniform1i(glGetUniformLocation(composite, "cloudColor"), 2);
        glUniform2f(
            glGetUniformLocation(composite, "depthProjection"), projection[10], projection[14]);
        glDrawArrays(GL_TRIANGLES, 0, 3);
        glDepthMask(GL_TRUE);
        glActiveTexture(GL_TEXTURE0);
    }
...
struct ScreenSpaceGI {
    ...
    explicit ScreenSpaceGI(const std::filesystem::path& directory) {
        ...
        try {
            composite = program(directory, "ssgi_composite");
        } catch (...) {
            glDeleteProgram(shader);
            throw;
        }
    ...
    void draw(GLuint sceneFbo,
        ...
        uniform(shader, "cameraRight", right);
        uniform(shader, "cameraUp", up);
        uniform(shader, "aspect", float(fullWidth) / fullHeight);
        uniform(shader, "tanHalfFov", std::tan(pi / 8));
...
struct PlanarReflection {
    ...
    void resize(int fullWidth, int fullHeight) {
        int nextWidth = std::max(1, (fullWidth + 1) / 2),
            nextHeight = std::max(1, (fullHeight + 1) / 2);
        if (nextWidth == width && nextHeight == height)
            return;
        width = nextWidth;
        height = nextHeight;
        auto allocate =
            [&](GLuint texture, GLenum internalFormat, GLenum format, GLenum type, GLint filter) {
                glBindTexture(GL_TEXTURE_2D, texture);
                glTexImage2D(
        ...
        allocate(color, GL_RGBA16F, GL_RGBA, GL_FLOAT, GL_LINEAR);
        allocate(depth, GL_DEPTH_COMPONENT24, GL_DEPTH_COMPONENT, GL_FLOAT, GL_NEAREST);
        glBindFramebuffer(GL_FRAMEBUFFER, fbo);
    ...
    void begin(int fullWidth, int fullHeight) {
        resize(fullWidth, fullHeight);
        glBindFramebuffer(GL_FRAMEBUFFER, fbo);
        glViewport(0, 0, width, height);
        glDrawBuffer(GL_COLOR_ATTACHMENT0);
...
struct Bloom {
    ...
    explicit Bloom(const std::filesystem::path& directory) {
        try {
            ...
            glDeleteProgram(downsample);
            glDeleteProgram(upsample);
            glDeleteProgram(composite);
            throw;
        }
        glGenVertexArrays(1, &vao);
        ...
        glGenFramebuffers(maxMips, fbos.data());
        glGenTextures(maxMips, colors.data());
    }
    ~Bloom() {
        glDeleteProgram(downsample);
        glDeleteProgram(upsample);
        ...
        glDeleteFramebuffers(maxMips, fbos.data());
        glDeleteTextures(maxMips, colors.data());
    }
    void resize(int w, int h) {
        if (w == width && h == height)
            return;
        ...
        for (int i = 0; i < maxMips; ++i) {
            ...
            mipWidths[i] = mw;
            mipHeights[i] = mh;
            allocate(fbos[i], colors[i], mw, mh);
            ++mipCount;
            if (mw < 4 || mh < 4)
                break;
        }
    ...
    void draw(GLuint scene,
        ...
        glUniform1i(glGetUniformLocation(composite, "scene"), 0);
        glActiveTexture(GL_TEXTURE1);
...
void run(GLFWwindow* window, const std::filesystem::path& directory) {
    struct Programs {
        ...
        ~Programs() {
            glDeleteProgram(terrain);
            glDeleteProgram(sky);
            glDeleteProgram(water);
            glDeleteVertexArrays(1, &skyVao);
            glDeleteVertexArrays(1, &waterVao);
        }
    } programs;
    startupDiagnostic("Initializing scene shaders");
    programs.terrain = program(directory, "terrain");
    programs.sky = program(directory, "sky");
    programs.water = program(directory, "water");
    glGenVertexArrays(1, &programs.skyVao);
    glGenVertexArrays(1, &programs.waterVao);
    startupDiagnostic("Generating terrain");
    std::random_device seedSource;
    terrainSeed = uint32_t(seedSource()) ^ (uint32_t(seedSource()) << 16);
    generateMountainRanges(terrainSeed);
    Terrain terrain;
    startupDiagnostic("Initializing particle simulation");
    Volcanoes volcanoes(directory, terrain);
    startupDiagnostic("Initializing lightning");
    Lightning lightning(directory);
    startupDiagnostic("Initializing UI renderer");
    UiRenderer ui(directory);
    startupDiagnostic("Initializing clouds");
    Clouds clouds(directory, volcanoes.gpu.terrainTexture);
    startupDiagnostic("Initializing rain");
    Rain rain(directory);
    startupDiagnostic("Initializing terrain shadows");
    TerrainShadows shadows(directory);
    startupDiagnostic("Initializing SSGI");
    ScreenSpaceGI screenSpaceGI(directory);
    startupDiagnostic("Initializing planar reflections");
    PlanarReflection reflection;
    startupDiagnostic("Initializing bloom");
    Bloom bloom(directory);
    startupDiagnostic("Startup complete; entering render loop");
    ...
    float terrainBrushRadius = 85.0f;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (ImGui::IsKeyPressed(ImGuiKey_Escape, false)) {
            if (placement != PlacementTool::None) {
                placement = PlacementTool::None;
                placementMiss = false;
            } else
                glfwSetWindowShouldClose(window, GLFW_TRUE);
        }
        bool placeClick = placement != PlacementTool::None && !io.WantCaptureMouse &&
                          ImGui::IsMouseClicked(ImGuiMouseButton_Left);
        bool targetClick = placement == PlacementTool::None && !io.WantCaptureMouse &&
                           ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left);
        if (placement != PlacementTool::None && !io.WantCaptureMouse)
            ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
        if (!io.WantCaptureMouse) {
            if (ImGui::IsMouseClicked(ImGuiMouseButton_Left) && placement == PlacementTool::None &&
                !targetClick)
                panning = true;
        ...
        Vec3 forward = orbit * (-1), right{ std::cos(yaw), 0, -std::sin(yaw) },
             up = cross(right, forward);
        if (panning) {
            // Match screen-space dragging at the orbit target, including on HiDPI displays.
            float unitsPerPixel =
                2 * distance * std::tan(pi / 8) / std::max(io.DisplaySize.y, 1.0f);
            target = target + right * (-io.MouseDelta.x * unitsPerPixel) +
                     up * (io.MouseDelta.y * unitsPerPixel);
        }
        Vec3 eye = target + orbit * distance;
        double wrappedDay = std::fmod(simulationTime / 60.0 + dayPhaseOffset, 1.0);
        if (wrappedDay < 0.0)
            wrappedDay += 1.0;
        float day = float(wrappedDay);
        ...
        Vec3 fog = Vec3{ 0.012f, 0.019f, 0.040f } * (1 - daylight) +
                   Vec3{ 0.42f, 0.59f, 0.72f } * daylight;
        ...
        if (particleSimulationEnabled)
            rain.update(elapsed,
                ...
                cloudBase,
                cloudTop,
                volcanoes.gpu.terrainTexture,
                clouds.densityTexture(),
                volcanoes.gpu.buffers[0],
                volcanoes.gpu.buffers[4],
                volcanoes.gpu.buffers[5]);

        shadows.render(terrain, sun);
        reflection.begin(w, h);
        Vec3 reflectedEye{ eye.x, 2.0f * waterLevel - eye.y, eye.z };
        Vec3 reflectedForward{ forward.x, -forward.y, forward.z };
        Vec3 reflectedUp{ up.x, -up.y, up.z };
        ...
        Vec3 reflectedRight{ right.x, -right.y, right.z };
        Mat4 reflectionProjection =
            perspective(float(reflection.width) / float(reflection.height), distance);
        Mat4 reflectionVp = multiply(reflectionProjection,
            lookAt(reflectedEye, reflectedForward, reflectedRight, reflectedUp));
        glDisable(GL_DEPTH_TEST);
        glDisable(GL_CULL_FACE);
        glDisable(GL_BLEND);
        glUseProgram(programs.sky);
        uniform(programs.sky, "cameraForward", reflectedForward);
        uniform(programs.sky, "cameraRight", reflectedRight);
        uniform(programs.sky, "cameraUp", reflectedUp);
        uniform(programs.sky, "aspect", float(reflection.width) / float(reflection.height));
        uniform(programs.sky, "tanHalfFov", std::tan(pi / 8));
        uniform(programs.sky, "sunDirection", sun);
        uniform(programs.sky, "daylight", daylight);
        uniform(programs.sky, "fogColor", fog);
        uniform(programs.sky, "atmosphereOpacity", atmosphereOpacity);
        uniform(programs.sky, "time", float(simulationTime));
        glBindVertexArray(programs.skyVao);
        glDrawArrays(GL_TRIANGLES, 0, 3);
        glEnable(GL_DEPTH_TEST);
        glEnable(GL_CLIP_DISTANCE0);
        glUseProgram(programs.terrain);
        glUniformMatrix4fv(glGetUniformLocation(programs.terrain, "viewProjection"),
            1,
            GL_FALSE,
            reflectionVp.data());
        uniform(programs.terrain, "eye", reflectedEye);
        uniform(programs.terrain, "sunDirection", sun);
        uniform(programs.terrain, "daylight", daylight);
        uniform(programs.terrain, "fogColor", fog);
        uniform(programs.terrain, "atmosphereOpacity", atmosphereOpacity);
        uniform(programs.terrain, "clipHeight", waterLevel);
        uniform(programs.terrain, "clipDirection", eye.y >= waterLevel ? 1.0f : -1.0f);
        rain.bindWetness();
        shadows.bind(programs.terrain);
        glBindVertexArray(terrain.vao);
        glDrawElements(GL_TRIANGLES, terrain.count, GL_UNSIGNED_INT, nullptr);
        float reflectionClipDirection = eye.y >= waterLevel ? 1.0f : -1.0f;
        volcanoes.draw(reflectionVp,
            shadows.lightMatrices[0],
            shadows.depth,
            0,
            reflectionVp,
            reflectedRight,
            ...
            true,
            waterLevel,
            reflectionClipDirection);
        rain.draw(reflectionVp,
            reflectedEye,
            reflectedRight,
            reflectedUp,
            daylight,
            ...
            reflectionClipDirection);
        lightning.draw(
            reflectionVp, reflectedEye, reflectedRight, true, waterLevel, reflectionClipDirection);
        ...
        glDisable(GL_CULL_FACE);
        glUseProgram(programs.sky);
        uniform(programs.sky, "cameraForward", forward);
        uniform(programs.sky, "cameraRight", right);
        uniform(programs.sky, "cameraUp", up);
        uniform(programs.sky, "aspect", float(w) / float(h));
        uniform(programs.sky, "tanHalfFov", std::tan(pi / 8));
        uniform(programs.sky, "sunDirection", sun);
        uniform(programs.sky, "daylight", daylight);
        uniform(programs.sky, "fogColor", fog);
        ...
        glUseProgram(programs.terrain);
        glUniformMatrix4fv(
            glGetUniformLocation(programs.terrain, "viewProjection"), 1, GL_FALSE, vp.data());
        uniform(programs.terrain, "eye", eye);
        uniform(programs.terrain, "sunDirection", sun);
        uniform(programs.terrain, "daylight", daylight);
        uniform(programs.terrain, "fogColor", fog);
        uniform(programs.terrain, "atmosphereOpacity", atmosphereOpacity);
        ...
        if (placeClick || targetClick) {
            ...
            if (px >= 0 && px < w && py >= 0 && py < h) {
                ...
                if (depth < 1) {
                    ...
                    if (targetClick)
                        target = position;
                    else if (placement == PlacementTool::TerrainUp ||
                             placement == PlacementTool::TerrainDown) {
                        float elevation = placement == PlacementTool::TerrainUp ? 18.0f : -18.0f;
                        terrain.deform(position, terrainBrushRadius, elevation);
                        volcanoes.terrainChanged(terrain);
                        placementMiss = false;
                    } else {
                        if (placement == PlacementTool::Volcano) {
                            position.y += Volcanoes::sourceClearance;
                            volcanoes.vents.push_back(position);
                        } else if (placement == PlacementTool::Spring) {
                            position.y += Volcanoes::sourceClearance;
                            volcanoes.springs.push_back(position);
                        } else
                            volcanoes.launchMeteor(position);
                        placement = PlacementTool::None;
                        placementMiss = false;
                    }
                }
        ...
        glUniformMatrix4fv(
            glGetUniformLocation(programs.water, "viewProjection"), 1, GL_FALSE, vp.data());
        uniform(programs.water, "eye", eye);
        uniform(programs.water, "sunDirection", sun);
        uniform(programs.water, "fogColor", fog);
        uniform(programs.water, "daylight", daylight);
        uniform(programs.water, "atmosphereOpacity", atmosphereOpacity);
        ...
        glUniform1i(glGetUniformLocation(programs.water, "reflectionTexture"), 6);
        glActiveTexture(GL_TEXTURE7);
        glBindTexture(GL_TEXTURE_2D, volcanoes.gpu.terrainTexture);
        glUniform1i(glGetUniformLocation(programs.water, "terrainHeight"), 7);
        glUniformMatrix4fv(glGetUniformLocation(programs.water, "reflectionViewProjection"),
            1,
            GL_FALSE,
            reflectionVp.data());
        ...
        if (!vaporAfterClouds)
            rain.draw(vp, eye, right, up, daylight);
        if (ssgiEnabled)
            clouds.beginEmission();
        volcanoes.draw(vp,
            shadows.lightMatrices[0],
            shadows.depth,
            reflection.color,
        ...
        if (ssgiEnabled) {
            clouds.endEmission();
            screenSpaceGI.draw(clouds.sceneFbo,
                clouds.sceneDepth,
                clouds.sceneEmission,
                w,
                h,
                eye,
                forward,
        ...
        if (cloudsEnabled) {
            ...
            glFramebufferTexture2D(
                GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, clouds.sceneDepth, 0);
            glDrawBuffer(GL_COLOR_ATTACHMENT0);
            glViewport(0, 0, w, h);
            if (vaporAfterClouds)
                volcanoes.drawVapor(vp,
                    shadows.lightMatrices[0],
                    shadows.depth,
                    reflection.color,
                    reflectionVp,
            ...
            lightning.draw(vp, eye, right);
        bloom.draw(cloudsEnabled ? bloom.hdrColor : clouds.sceneColor,
            bloomEnabled,
            std::pow(2.0f, cameraExposure),
            bloomIntensity,
            framebufferWidth,
            framebufferHeight);
        ...
        if (ImGui::SliderFloat("Time of day",
            ...
            dayPhaseOffset += timeOfDayHours / 24.0f - day;
        ImGui::SetNextItemWidth(180 * uiScale);
        ImGui::SliderFloat(
            "Time speed", &timeSpeed, 0.0f, 4.0f, "%.2fx", ImGuiSliderFlags_AlwaysClamp);
        if (timeSpeed > 0)
            ImGui::TextDisabled("1 day = %.1f real seconds", 60.0f / timeSpeed);
        else
        ...
        ImGui::SetNextItemWidth(180 * uiScale);
        ImGui::SliderFloat(
            "Bloom intensity", &bloomIntensity, 0.0f, 3.0f, "%.2fx", ImGuiSliderFlags_AlwaysClamp);
        ...
        if (ImGui::Button("Terrain up")) {
            placement = PlacementTool::TerrainUp;
            placementMiss = false;
            panning = false;
        }
        ImGui::SameLine();
        ...
        ImGui::SliderFloat("Terrain brush radius",
            ...
            ImGuiSliderFlags_AlwaysClamp);
        if (placement != PlacementTool::None) {
            const char* placementPrompt =
                placement == PlacementTool::Volcano     ? "Click terrain to place a volcano."
                : placement == PlacementTool::Spring    ? "Click terrain to place a spring."
                : placement == PlacementTool::Meteor    ? "Click terrain to target a meteor."
                : placement == PlacementTool::TerrainUp ? "Click terrain to raise it."
            ...
            if (placementMiss)
                ImGui::TextColored(
                    ImVec4(1, 0.65f, 0.3f, 1), "No terrain here. Click the landscape.");
        }
        ImGui::SetNextItemWidth(180 * uiScale);
        ImGui::DragFloat(
            "Particle life", &volcanoes.particleLifetime, 1.f, 1.0f, 3600.0f, "%.1f s");
        ImGui::SetNextItemWidth(180 * uiScale);
        ImGui::SliderFloat("Particle brightness",
            &volcanoes.particleBrightness,
            0.0f,
            100.0f,
            "%.2fx",
            ImGuiSliderFlags_AlwaysClamp);
        ...
        ImGui::SetNextItemWidth(180 * uiScale);
        ImGui::SliderFloat(
            "Water level", &waterLevel, -100.0f, 250.0f, "%.1f", ImGuiSliderFlags_AlwaysClamp);
        ImGui::SetNextItemWidth(180 * uiScale);
        ImGui::DragFloat(
            "Lightning frequency", &lightning.frequency, 1.f, 0.f, 1.e6f, "%.1f / min");
        ImGui::SetNextItemWidth(180 * uiScale);
        ...
        ImGui::SetNextItemWidth(180 * uiScale);
        ImGui::SliderFloat(
            "Cloud coverage", &cloudCoverage, 0.0f, 1.0f, "%.2f", ImGuiSliderFlags_AlwaysClamp);
        ImGui::SetNextItemWidth(180 * uiScale);
        ImGui::SliderFloat(
            "Cloud opacity", &cloudOpacity, 0.0f, 1.0f, "%.2f", ImGuiSliderFlags_AlwaysClamp);
        ImGui::SetNextItemWidth(180 * uiScale);
        ImGui::SliderFloat(
            "Wind speed", &windSpeed, 0.0f, 500.0f, "%.1f units/s", ImGuiSliderFlags_AlwaysClamp);
        ImGui::SetNextItemWidth(180 * uiScale);
        ImGui::SliderFloat(
            "Cloud height", &cloudBase, 0.0f, 1000.0f, "%.0f", ImGuiSliderFlags_AlwaysClamp);
...
int main(int argc, char** argv) {
    ...
    int initialWidth = videoMode ? videoMode->width : 1440;
    int initialHeight = videoMode ? videoMode->height : 900;
    GLFWwindow* window = glfwCreateWindow(
        initialWidth, initialHeight, "EarthSim", videoMode ? monitor : nullptr, nullptr);
    if (!window) {
        glfwTerminate();
        return 1;
    }
    ...
    startupDiagnostic(std::string("OpenGL: ") + (version ? version : "unknown") +
    ...
    int result = 0;
    try {
        run(window, shaderDirectory(argv[0]));
